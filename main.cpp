...
int main(int argc, char* argv[]){
	if (argc!=2) printf("Usage: SimPiMIPS <filename>\n");

	Memory instMem,dataMem;
	Register reg;
	CPU cpu(&reg,&instMem,&dataMem);
	...
	while (reg.readPC()<4*20){
		cpu.exec();
		/*printf("r2=%d, r5=%d, r6=%d, r1=%d, r7=%d, r15=%d, r16=%d mem=%d\n",
				reg.read(2),reg.read(5),reg.read(6),reg.read(1),reg.read(7),reg.read(15),reg.read(16),dataMem.read(5000+4*reg.read(1)))*/;
	}


	for (int i=0; i<1000; i++){
		if (2*x[i]+y[i]!=dataMem.read(5000+4*i)) printf("errado\n");
	}
